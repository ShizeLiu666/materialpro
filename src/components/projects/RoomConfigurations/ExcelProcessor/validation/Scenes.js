...
function validateDimmerTypeOperations(names, operation, errors, sceneName) {
    ...
    if (
      ...
      return; // Skip further checks if the format is wrong
    }
  
    // Check if the dimming value exceeds 100%
    const dimmerValueMatch = operation.match(/\+(\d+)%$/);
    if (dimmerValueMatch) {
      const dimmerValue = parseInt(dimmerValueMatch[1], 10);
      if (dimmerValue > 100) {
        errors.push(`KASTA SCENE [${sceneName}]: Dimmer Type operation '${operationString}' has a dimming value exceeding 100%. Please provide a value between 0% and 100%.`);
      }
    }
  }  
...
function validateSceneDevicesInLine(
  ...
  parts.forEach((part) => {
    if (["ON", "OFF", "OPEN", "CLOSE"].includes(part)) {
      // 如果 `operation` 已经被设置过，这里不会覆盖，但我们会继续处理。
      if (!operation) {
        operation = part; // 第一次赋值操作符
      }
      deviceNames.push([]);
    } else if (/^\+\d+%$/.test(part)) {
  ...
  if (
    ...
    console.log(
      "Calling validateDimmerTypeOperations with:",
      deviceNames.flat(),
      operation
    );
  ...
  sceneDataArray.forEach((line) => {
    ...
    if (line.startsWith("NAME")) {
      ...
      validateSceneDevicesInLine(
        ...
        deviceNameToType,
        currentSceneName
      ); // Pass the scene name to the validation function
    }
  });
