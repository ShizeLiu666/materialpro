...
function checkCommandFormat(line, errors, currentRemoteControlName, maxKeyCount) {
    ...
    if (!/(DEVICE|GROUP|SCENE)/.test(command)) {
        errors.push(
            `KASTA REMOTE CONTROL: The command '${command}' in '${currentRemoteControlName}' must contain one of the following: 'DEVICE', 'GROUP', 'SCENE'.`
        );
        return false;
    }
...
function validateGroupCommand(command, errors, currentRemoteControlName, registeredGroupNames) {
    const groupMatch = command.match(/^GROUP\s+(\S+)(?:\s+-\s+(\S+))?$/);
    if (!groupMatch) {
        errors.push(
            `KASTA REMOTE CONTROL: The GROUP command '${command}' in '${currentRemoteControlName}' is not valid. Expected format: 'GROUP <group_name>' with an optional operation after a space and '-'.`
        );
        return false;
    }
...
function validateSceneCommand(command, errors, currentRemoteControlName, registeredSceneNames) {
    const sceneMatch = command.match(/^SCENE\s+(\S+)(?:\s+-\s+(\S+))?$/);
    if (!sceneMatch) {
        errors.push(
            `KASTA REMOTE CONTROL: The SCENE command '${command}' in '${currentRemoteControlName}' is not valid. Expected format: 'SCENE <scene_name>' with an optional operation after a space and '-'.`
        );
        return false;
    }
...
export function validateRemoteControls(remoteControlDataArray, deviceNameToType, registeredDeviceNames, registeredGroupNames, registeredSceneNames) {
    ...
    remoteControlDataArray.forEach((line) => {
        ...
        if (line.startsWith("NAME")) {
            ...
            if (!validateRemoteControlName(currentRemoteControlName, errors, registeredRemoteControlNames)) return;
